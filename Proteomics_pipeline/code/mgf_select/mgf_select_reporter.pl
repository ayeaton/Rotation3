...
else
{
	...
	else
	{
		...
		else
		{
			if ($type=~/^TMT10$/)
			{
				@reporters=(126.127725,127.12476,127.131079,128.128114,128.134433,129.131468,129.137787,130.134822,130.141141,131.138176);
			}
			else
			{
				print "Error\n";
		...
		foreach my $filename (@allfiles)
		{
			if ($filename=~/\.mgf$/i)
			{
				...
				if(open (IN, "$dir/$filename"))
				{
					...
					{
						open (OUT_TABLE,">$dir/selected-d$mz_error-$type-$min_intensity-$min_reporters/$filename.txt");
						print OUT_TABLE qq!filename\tscan\trt\tMS1_intensity!;
						foreach my $reporter (@reporters)
						{
							my $reporter_=int($reporter);
							...
							else
							{
								if ($done_reading_fragments==1)
								{
									...
									if ($reporters_found>=$min_reporters)
									{
										...
										print OUT $footer;
										print OUT_TABLE qq!$filename\t$scans\t$rt\t$ms1_intensity!;
